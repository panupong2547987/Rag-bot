...
export async function POST(req: Request) {
  ...
  const yearsFound = [
  ...new Set(finalContext.match(/25\d{2}/g) || [])
].sort()

const requestedYear = userQuestion.match(/25\d{2}/)?.[0]

const isCalendarQuestion =
  /ปฏิทิน|เปิดเทอม|ปิดเทอม|ภาคเรียน|เทอม|วันเรียน/.test(userQuestion)
...
if (
  ...
  return Response.json({
    answer: `ระบบพบข้อมูลปีการศึกษา ${yearsFound.join(", ")} คุณต้องการปีใดครับ`,
    contextMatches: docs.length,
    fallbackUsed: false,
    fallbackReason: "multiple-years",
    model: "year-check"
  })
}
...
- หากผู้ใช้ถามคำถามแบบกำกวมโดยไม่ระบุปี
และข้อมูลอ้างอิงมีมากกว่า 1 ปีการศึกษา

  ให้ทำตามนี้:
  1. ห้ามเลือกตอบปีใดปีหนึ่งเอง
  2. ต้องแจ้งปีการศึกษาที่พบทั้งหมด
  3. ต้องถามผู้ใช้ให้เลือกปีก่อนตอบเสมอ

  ตัวอย่าง:
  "ระบบพบข้อมูลปีการศึกษา 2568, 2569, 2570 คุณต้องการปีใดครับ?"
...
- 🚀 [โหมดใหม่สำคัญ]
  ...
  และข้อมูลอ้างอิงเป็นปีการศึกษาเดียวกัน
  แต่มีทั้งเทอม 1 และเทอม 2

  👉 ห้ามถามกลับ
  👉 ต้องแสดงข้อมูลทั้งสองเทอมทันที
...
- หากผู้ใช้ระบุปีมาแล้ว
  ให้ตอบเฉพาะปีที่ผู้ใช้ระบุเท่านั้น

- ห้ามสร้างหรือเดาปีการศึกษาที่ไม่มีอยู่ในข้อมูลอ้างอิง

- ถ้าผู้ใช้ทักทาย ให้ตอบทักทายตามธรรมชาติ
...
- หลีกเลี่ยงประโยคปัดสั้น ๆ เช่น
  "ไม่มีข้อมูลที่ match ได้เลย"

- ถ้าข้อมูลไม่พอ
  ให้บอกว่าขาดอะไร
  และยกตัวอย่างสิ่งที่ผู้ใช้ควรระบุเพิ่ม

- ถ้าไม่มั่นใจ
  👉 ห้ามเดาเด็ดขาด
  👉 ให้ถามข้อมูลเพิ่มก่อน

- ถ้าในข้อมูลอ้างอิงมีคำตอบตรง
  ให้ตอบจากข้อมูลอ้างอิงก่อนเสมอ
  และห้ามแต่งข้อมูลเพิ่ม

- โหมดคำตอบตอนนี้คือ:
  ${answerMode}
  (strict = เข้มที่สุด, balanced = ตรงข้อมูลแต่เป็นธรรมชาติ, chat = อธิบายเพิ่มได้)
